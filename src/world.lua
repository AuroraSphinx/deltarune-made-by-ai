...
function World:collidesAt(x, y)
    ...
    for _, wall in ipairs(self:getRoom().walls) do
...
function World:moveAxis(dx, dy)
    if dx ~= 0 then
        ...
        if not self:collidesAt(nextX, self.player.y) then
            self.player.x = nextX
        end
    end
    ...
    if dy ~= 0 then
        ...
        if not self:collidesAt(self.player.x, nextY) then
            self.player.y = nextY
        end
    end
end
...
function World:update(dt)
    ...
    if self.transitionCooldown <= 0 then
        ...
        for _, exit in ipairs(self:getRoom().exits) do
...
function World:interact()
    ...
    for _, npc in ipairs(self:getRoom().npcs) do
...
local function drawStarField(seed, count, offsetX)
    love.math.setRandomSeed(seed)
    for _ = 1, count do
        local x = love.math.random(20, 300) + (offsetX or 0)
        local y = love.math.random(20, 215)
        local size = love.math.random(1, 2)
        love.graphics.rectangle("fill", x, y, size, size)
    end
end

function World:drawField()
    love.graphics.clear(0.055, 0.025, 0.11, 1)

    love.graphics.setColor(0.34, 0.18, 0.50, 1)
    drawStarField(1337, 42)
...
function World:draw()
    ...
    for _, npc in ipairs(room.npcs) do
        self.assets:draw(npc.sprite, npc.x, npc.y)
    end

    self.assets:draw("hero_" .. self.player.direction, self.player.x - 2, self.player.y - 4)
