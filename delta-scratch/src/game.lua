...
local displayConfig = CONFIG.display or {}
local pathConfig = CONFIG.paths or {}

local Game = {
    virtualWidth = displayConfig.virtual_width or 320,
    virtualHeight = displayConfig.virtual_height or 240,
    state = "title",
    titlePage = "main",
    titleMenuIndex = 1,
    optionsIndex = 1,
...
-- Fonts are rasterized at `size * dpiscale` while keeping layout metrics at
-- `size`, so text stays razor sharp when the virtual world is scaled up to
-- the window resolution (see Game:resize).
local function loadFont(path, size, filter, dpiscale)
    local font
    if love.filesystem.getInfo(path, "file") then
        font = love.graphics.newFont(path, size, "normal", dpiscale or 1)
    else
        font = love.graphics.newFont(size, "normal", dpiscale or 1)
    end
    if filter then
...
function Game:sortResolutions()
    table.sort(self.resolutions, byArea)
end
...
function Game:loadSettings()
    local currentWidth, currentHeight = love.graphics.getDimensions()
    -- Default scaling from config: linear -> smooth, integer_scaling -> pixel,
    -- otherwise sharp.
    local defaultScaling = "sharp"
    if displayConfig.filter == "linear" then
        defaultScaling = "smooth"
    elseif displayConfig.integer_scaling then
        defaultScaling = "pixel"
    end
...
-- Window-space fonts for the native title/options UI. Sizes are already
-- scaled by uiScale, so these are rasterized 1:1 (dpiscale 1).
...
-- Virtual-space fonts used by the world, dialogue and battle. The table is
-- mutated in place so existing references (dialogue/world/battle) stay valid.
-- dpiscale matches the internal scale, giving crisp text at any resolution.
function Game:rebuildVirtualFonts(scale)
    local fonts = self.fonts
    if not fonts then return end
    fonts.tiny = loadFont(self.monoFontPath, 8, nil, scale)
    fonts.small = loadFont(self.monoFontPath, 10, nil, scale)
    fonts.normal = loadFont(self.monoFontPath, 12, nil, scale)
    fonts.title = loadFont(self.sansFontPath, 24, nil, scale)
    fonts.subtitle = loadFont(self.sansFontPath, 12, nil, scale)
end

function Game:applyRenderFilter()
    local filter = "nearest"
    if self.settings and self.settings.scaling == "smooth" then
        filter = "linear"
    end
    ...
    for _, font in pairs(self.fonts or {}) do
        font:setFilter(filter, filter)
    end
    if self.assets and self.assets.setFilter then
        self.assets:setFilter(filter)
    end
    if self.world and self.world.setFilter then
        self.world:setFilter(filter)
    end
end
...
function Game:load()
    local fontsPath = pathConfig.fonts or "assets/fonts"
    self.monoFontPath = fontsPath .. "/DeterminationMonoWebRegular-Z5oq.ttf"
    self.sansFontPath = fontsPath .. "/DeterminationSansWebRegular-369X.ttf"

    -- Window icon (small + taskbar big). Guarded: no-ops on web builds.
    pcall(function()
        love.window.setIcon(
            love.image.newImageData(CONFIG.window and CONFIG.window.icon or "icon.png"),
            love.image.newImageData(CONFIG.window and CONFIG.window.bigicon or "bigicon.png")
        )
    end)

    -- Base-size virtual fonts; resize() replaces the entries with
    -- high-DPI versions so world text is crisp at any internal scale.
    self.fonts = {
...
-- The render target is the full window, not the 320x240 virtual grid: the
-- world is drawn through a scale transform, so shapes, text and UI rasterize
-- at native resolution instead of being upscaled from a tiny canvas.
function Game:resize(width, height)
    ...
    self.scale = scale
    self.offsetX = math.floor((width - self.virtualWidth * scale) / 2)
    self.offsetY = math.floor((height - self.virtualHeight * scale) / 2)

    if self.canvas then
        self.canvas:release()
    end
    self.canvas = love.graphics.newCanvas(width, height)
    self:rebuildVirtualFonts(scale)
